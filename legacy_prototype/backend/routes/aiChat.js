...
router.post('/send', requireAuth, async (req, res) => {
    ...
    `).run(uuidv4(), userId, newBalance);
      ...
      `).run(
            ...
            // Create proof for the fraud alert to sync global chain (FIX: ghost block issue)
            const fraudProofObj = createProof(fraudMsgId, 'SYSTEM', `[FRAUD ALERT] ${severity}`, channelId);
