  ...
  setImmediate(async () => {
    try {
      ...
      const result = await route({
        goal: content.replace(/@[a-zA-Z0-9_-]+\s*/g, '').trim() + roomContext,
        userId,
      ...
      if (!others.length && MAX_FOLLOWUPS > 0) {
...
// A mentioned secondary agent must answer (not optional like maybeFollowUp)
async function maybeFollowUpDirect(groupId, agentId, userContent) {
  ...
  const messages = [
    {
      role: 'system',
      content: `You are ${cfg.name}, a specialist agent (${(cfg.capabilities || []).join(', ')}) in a FinChat group discussion. ` +
        `The user explicitly mentioned you. Answer their message from YOUR specialty's perspective in a short, direct reply (max 5 sentences).`
    },
    { role: 'user', content: `${transcriptFor(history)}\n\nAnswer the user's message: "${userContent}"` }
  ];
  try {
